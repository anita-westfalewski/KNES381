...
def play_game():
  ...
  while not winner and not draw:
    ...
    print("It is player", player,"'s turn to play!")
    #subtract one from the row and col to turn input into number that works for the rest of code
    row = int(input("Enter row (1-3): "))
    col = int(input("Enter column (1-3): "))
    row = row - 1
    col = col -1

    #do not let people to override their opponent's move
    if board[row][col] != "_":
      print("You cannot play on a space that has already been played on! Try another spot.")
      continue
